...
<div class="filter-product <?php echo esc_attr($filter_style.' '.$filter_column.' '.$filter_pos)?>" data-load="<?php echo esc_attr($data_loadjs);?>" >
    <a href="#" class="btn-filter"><span class="color"><i class="la la-search"></i></span><?php esc_html_e("Filter","construction");?></a>
    <div class="box-attr-filter">
        ...
        ?>
            <div class="item-box-attr">
                <div class="item-attr-title">
                    <label><?php esc_html_e("Categories","construction");?></label>
                </div>
        ...
        <?php
        if($filter_price == 'yes'){
            $price_arange = th_get_price_arange();
            $min = $price_arange['min'];
            $max = $price_arange['max'];
            ?>
            <div class="item-box-attr">
                <div class="item-attr-title">
                    <label><?php esc_html_e("Price","construction");?></label>
                </div>
                <div class="item-attr-content range-filter">
                    ...
                    <div class="attr-price-filter">
                        <p>
                            <label><?php esc_html_e("Filter By Price:","construction");?></label>
        ...
        if(!empty($filter_attr)):
            ...
            foreach($attribute_filter as $attr_filter):
                ...
                $attr_tax = th_Woocommerce_Attributes::th_get_tax_attribute( "pa_".$attr_filter);
                                            ...
                                            if(is_object($term)){$value = get_term_meta( $term->term_id, 'image', true );
                                                ...
                                                if(!empty($image)){
                                                    echo sprintf(
                                                        ...
                                                        esc_attr( $name ),
                                                        esc_attr( $term->slug ),
                                                        esc_url( th_get_filter_url('filter_'.$attr_tax->attribute_name,$term->slug) ),
                                                        esc_url( $image ),
                                                        esc_attr( $name ),
                                                        esc_attr( $name )
                                                    );
                                        ...
                                        foreach ($terms as $term){
                                            if(is_object($term)){
                                                ...
                                                if(!empty($color)){
                                                    ...
                                                    echo sprintf(
                                                        '<a data-attribute="%s" data-term="%s" class="element-filter swatch swatch-color '.$class_white_color.' swatch-%s %s" '.th_add_html_attr('background-color:'.$color).' title="%s" href="%s"><span class="hide">%s</span></a>',
                                                        esc_attr($attr_filter),
                                                        esc_attr( $term->slug ),
                                                        esc_attr( $term->slug ),
                                                        $selected,
                                        ...
                                        foreach ($terms as $term) {
                                            if(is_object($term)){
                                                ...
                                                echo    '<li class="'.esc_attr($term->slug).'-inline">
                                                            <a data-attribute="'.esc_attr($attr_filter).'" data-term="'.esc_attr($term->slug).'" class="element-filter '.esc_attr($active).' bgcolor-'.esc_attr($term->slug).'" href="'.esc_url(th_get_filter_url('filter_'.$attr_filter,$term->slug)).'">
                                                            <span></span>'.$term->name.'
                                                            </a>
                                                        </li>';
                                            }
