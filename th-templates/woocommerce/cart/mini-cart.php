...
<?php if ( ! WC()->cart->is_empty() ) : ?>
                    ...
                    <div class="item-info-cart product-mini-cart table-custom <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>" data-key="<?php echo esc_attr($cart_item_key)?>">
                        ...
                        <div class="product-info">
                            ...
                            <div class="mini-cart-qty">
                                <span><?php echo apply_filters( 'woocommerce_widget_cart_item_quantity', '<span class="qty-num">' . sprintf( '%s', $cart_item['quantity'] ) . '</span>', $cart_item, $cart_item_key ); ?> x <span class="color"><?php echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key );?></span></span>
                            </div>
                        </div>
                        <div class="product-delete text-right">
                            <a href="#" class="remove-product"><i class="la la-trash"></i></a>
                        </div>
                    </div>
                    <?php
                }
    ...
    <div class="mini-cart-total text-uppercase title18 clearfix">
        ...
        <strong class="pull-right color mini-cart-total-price get-cart-price"><?php echo WC()->cart->get_cart_subtotal(); ?></strong>
    </div>
