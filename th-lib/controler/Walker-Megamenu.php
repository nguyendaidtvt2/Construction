	...
	// add main/sub classes to li's and links
 	function start_el(  &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
	    $indent = ( $depth > 0 ? str_repeat( "\t", $depth ) : '' ); // code indent
	  	...
	  	if(!empty($content_item)) $content = th_Template::get_vc_pagecontent($content_item);	  	
	    ...
	    $item_output = '';
	    if(is_object($args)){
	    	if(in_array('menu-item-has-children', $item->classes) || $mega_menu) $indicator_html = '<i class="indicator-icon"></i>';
	    	else $indicator_html = '';
		    $item_output = sprintf( '%1$s<a%2$s>'.$icon_html .'%3$s%4$s'.$indicator_html.'%5$s</a>%6$s',
		        $args->before,
		        $attributes,
		        $args->link_before,
		    ...
		    if(!empty($image))  $item_output .= '<div class="preview-image">
	  		...
	  		if($mega_menu){
		    	...
		    	$content = str_replace('http'.'s://construction.ththeme.net/', esc_url(home_url('/')), $content);
		    	$content = str_replace('http'.'://construction.ththeme.net/', esc_url(home_url('/')), $content);
		    	$output .= $indent . '<li id="nav-menu-item-'. $item->ID . '" class="' . $depth_class_names . ' ' . $class_names . '">';
	    		$output .= apply_filters( 'walker_nav_menu_start_el', $item_output, $item, $depth, $args );
	    		if($depth == 0) $output .= '<div class="mega-menu" '.th_add_html_attr('width:'.esc_attr($width)).'>'.do_shortcode($content).'</div>';
	    	}	
