<?php
if(!class_exists('Th_Woocommerce_Attributes')){
    class Th_Woocommerce_Attributes{
        static function _init(){
            if (!class_exists('WC_Product')) return;

            //Add Type Attributes Woo
            add_action( 'woocommerce_product_option_terms', array( __CLASS__, 'th_product_option_terms_attribute' ), 10, 2 );
            if(is_admin()){
                add_filter( 'product_attributes_type_selector', array( __CLASS__, 'th_add_attribute_types' ) );
                add_action('admin_enqueue_scripts', array(__CLASS__, 'th_attributes_admin_scripts'));
                add_action('admin_init', array(__CLASS__, 'th_init_attribute_hooks'));
                add_action( 'th_product_attribute_field', array( __CLASS__, 'th_attribute_fields' ), 10, 3 );
            }
            //Font end attribute
            add_filter( 'woocommerce_dropdown_variation_attribute_options_html', array( __CLASS__, 'th_get_swatch_html_attribute' ), 100, 2 );
            add_filter( 'th_filters_swatch_html_attribute', array( __CLASS__, 'th_swatch_html_attribute' ), 5, 4 );
        ...
        static function th_get_tax_attribute( $taxonomy ) {
            global $wpdb;
        ...
        static function th_product_option_terms_attribute( $taxonomy, $index ) {
            $types = array(
                'color' => esc_html__( 'Color', 'construction' ),
                'image' => esc_html__( 'Image', 'construction' ),
            ...
            <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'construction' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr($index); ?>][]">
                <?php
            ...
            <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'construction' ); ?></button>
            <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'construction' ); ?></button>
            <button class="button fr plus tawcvs_add_new_attribute" data-type="<?php echo esc_attr($taxonomy->attribute_type); ?>"><?php esc_html_e( 'Add new', 'construction' ); ?></button>

            <?php
        }
        static function th_init_attribute_hooks() {
            ...
            add_action( 'created_term', array( __CLASS__, 'th_save_term_meta_attribute' ), 10, 2 );
            add_action( 'edit_term', array( __CLASS__, 'th_save_term_meta_attribute' ), 10, 2 );
        }
        static function s7up_add_attribute_fields( $taxonomy ) {
            $attr = th_Woocommerce_Attributes::th_get_tax_attribute( $taxonomy );
            do_action( 'th_product_attribute_field', $attr->attribute_type, '', 'add' );
        }
        static function s7up_edit_attribute_fields( $term, $taxonomy ) {
            $attr = th_Woocommerce_Attributes::th_get_tax_attribute( $taxonomy );
            $value = get_term_meta( $term->term_id, $attr->attribute_type, true );

            do_action( 'th_product_attribute_field', $attr->attribute_type, $value, 'edit' );
        }
        static function th_attribute_fields( $type, $value, $form ) {
            ...
            switch ( $type ) {
                case 'image':
                    ...
                    <div class="wrap-metabox">
                        ...
                        <a class="button button-primary sv-button-remove "> <?php esc_html_e("Remove",'construction')?></a>
                        <a class="button button-primary sv-button-upload-id"><?php esc_html_e("Upload",'construction')?></a>
        ...
        static function th_save_term_meta_attribute( $term_id, $tt_id ) {
            $types = array(
                'color' => esc_html__( 'Color', 'construction' ),
                'image' => esc_html__( 'Image', 'construction' ),
                'label' => esc_html__( 'Label', 'construction' ),
        ...
        static function th_add_attribute_types($types) {
            $add_type = array(
                'color' => esc_html__( 'Color', 'construction' ),
                'image' => esc_html__( 'Image', 'construction' ),
                'label' => esc_html__( 'Label', 'construction' ),
        ...
        static function th_attributes_admin_scripts(){
        ...
        static function th_get_swatch_html_attribute( $html, $args ) {
            $swatch_types = array(
                'color' => esc_html__( 'Color', 'construction' ),
                'image' => esc_html__( 'Image', 'construction' ),
                'label' => esc_html__( 'Label', 'construction' ),
            );
            $attr         = th_Woocommerce_Attributes::th_get_tax_attribute( $args['attribute'] );
                ...
                if ( ! empty( $options ) && $product && taxonomy_exists( $attribute ) ) {
                    ...
                    foreach ( $terms as $term ) {
                        if ( in_array( $term->slug, $options ) ) {
                            $swatches .= apply_filters( 'th_filters_swatch_html_attribute', '', $term, $attr, $args );
                        }
                    }
                }
        ...
        static function th_swatch_html_attribute( $html, $term, $attr, $args ) {
            ...
            switch ( $attr->attribute_type ) {
                case 'color':
                    ...
                    $html = sprintf(
                        '<span class="swatch swatch-color '.$class_white.' swatch-%s %s" '.th_add_html_attr('background-color:'.$color).' title="%s" data-value="%s"><span class="hide">%s</span></span>',
                        esc_attr( $term->slug ),
                        $selected,
                        esc_attr( $name ),
                        esc_attr( $term->slug ),
    ...
    Th_Woocommerce_Attributes::_init();
}