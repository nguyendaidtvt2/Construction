...
add_action( 'comment_form_before', 'th_comment_form_before');
if(!function_exists('th_comment_form_before')){
    function th_comment_form_before(){
        $fields = apply_filters( 
            'wpse_comment_fields', 
            array( 'comment', 'author', 'url', 'email', 'submit' )
        );
        $o = new Th_Rearrange_Comment_Fields;
        $o->set_fields( $fields )->init();
    }
}
class Th_Rearrange_Comment_Fields
{
    private $html       = array();
    ...
    public function display()
    {
        ...
        echo apply_filters('th_output_content',$html);
    }
...
add_filter( 'wpse_comment_fields', 'th_comment_fields');
if(!function_exists('th_comment_fields')){
    function th_comment_fields($fields){
